...
def partial_display(parent1, parent2):
    ...
    def pmx(parent, split, display_start, display_end):
        descendant = parent[:split] + [0] * len(parent[split:-split]) + parent[-split:]

        for _ in range(len(descendant)):
            for i in range(len(display_start)):
                if display_start[i] in descendant:
                    descendant[descendant.index(display_start[i])] = display_end[i]

        descendant[split:-split] = display_start[:]
        return descendant

    split = len(parent1) // 3
    display_start, display_end = tuple(parent1[split:-split]), tuple(parent2[split:-split])  # пары отображений

    descendant1 = pmx(parent2, split, display_start, display_end)
    descendant2 = pmx(parent1, split, display_end, display_start)
...
def partial_return_selection(population, p_size):
    ...
    new_population = []
    fines = [el[1] / 100 for el in population]
    sum_fines = sum(fines)
...
filename = 'time.csv'
filename_result_matrix = f'result_EGA_{datetime.now().strftime("%d.%m.%Y_%H.%M.%S")}.csv'
deep = 0
population_size = 14
...
print(f'Начальные параметры:\n'
        f'Глубина: {deep if deep != 0 else "до совпадения средних приспособленностей"}\n'
...
def ega(filename, deep, population_size, probability_mutation, fights, elite_selection,
    ...
    population = create_start_populations(population_size, filename)
    ...
    for i in range(deep):
        ...
        if min_fine > max_fitness[len(max_fitness) - 1]:
            min_fine = max_fitness[len(max_fitness) - 1]
            result_permunation = population[fitness.index(min_fine)][:]

        if deep == 10**100:
            if len(mean_fitness) == 1 or previous_mean_fitness != mean_fitness[i]:
                previous_mean_fitness = mean_fitness[i]
            else:
                break

        print(f'Популяция {i + 1}, Макс. присп: {max_fitness[i]}, Средняя присп: {mean_fitness[i]}')
        # для проверки!!!
        print(*fitness)
        print()

    # with open('result_EGA.csv', 'a', encoding='utf-8', newline='') as file:
    #     writer = csv.writer(file, delimiter=';')
    #     writer.writerow((choose_parents_method, breeding_method, associative_crossing_method,
    #                      crossover_method, mutation_method, macromutation_method, probability_mutation, algorithm_copy,
    #                      selection_scheme, elite_selection, fights, scaling, max_fitness[len(max_fitness) - 1],
    #                      mean_fitness[len(mean_fitness) - 1]))

    #Methods.write_result(filename_result_matrix, min_fine, result_permunation, times)
...
# import csv
# with open('result_EGA.csv', 'w', encoding='utf-8', newline='') as file:
#     writer = csv.writer(file, delimiter=';')
...
ega(filename, deep, population_size, probability_mutation, fights, elite_selection,
        algorithm_copy, choose_parents_method, breeding_method, associative_crossing_method,
        crossover_method, mutation_method, macromutation_method, scaling)
