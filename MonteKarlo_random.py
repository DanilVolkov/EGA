...
filename_result_matrix = f'result_Monte_Karlo_random_{datetime.now().strftime("%d.%m.%Y_%H.%M.%S")}.csv'       # имя файла для вывода результата
...
def random_permutation():
    '''Функция генерирует перестановку, которой нет в random_permutations.'''

    permutation = []                            # результирующая перестановка

    while not permutation:
        while len(permutation) < tasks:
            n = randint(1, tasks)
            while n in permutation:
                n = randint(1, tasks)
            permutation.append(n)
        if permutation in random_permutations:
            permutation = []
